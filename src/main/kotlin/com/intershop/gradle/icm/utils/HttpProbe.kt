...
class HttpProbe(target : URI) {
    ...
    fun execute(): Boolean {
        ...
        while (!success){
            ...
            if (dur > retryTimeout){
                logger.warn("Failed to probe a HTTP {} to {} (total retry duration is {})", request.method(), request.uri(), dur)
                // if so fail
                return false
            }
            success = executeOnce()
        ...
        logger.info("Successfully probed a HTTP {} to {} (total duration is {})", request.method(), request.uri(), totalDuration)
        return true
    }
    ...
    private fun executeOnce(): Boolean {
        ...
        logger.debug("Received response while probing a HTTP {} to {}: status={}", request.method(), request.uri(), response.statusCode())
