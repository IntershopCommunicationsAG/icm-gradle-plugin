...
open class PrepareSitesFolder
    ...
    init {
        outputDir.set(File(project.projectDir, "${CROSSPRJ_FOLDERPATH}/storefront/sites"))
    }
    ...
    override fun addPackages(cs: CopySpec) {
        ...
        modules.get().forEach { prj ->
            ...
            val dir = if(key.isNotBlank()) { moduleDirectories.get().get(key) } else { null }
