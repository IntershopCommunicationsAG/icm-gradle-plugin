...
class ICMProjectPluginIntegrationSpec extends AbstractIntegrationGroovySpec {
    ...
    def 'check simple cartridge list and setup cartridges'() {
        ...
        def resultDEVECL = getPreparedGradleRunner()
                .withArguments(com.intershop.gradle.icm.ICMProjectPlugin.EXTEND_DEVCARTRIDGELIST, "-s")
                .withGradleVersion(gradleVersion)
                .build()
        ...
        def resultDEVSC = getPreparedGradleRunner()
                .withArguments(com.intershop.gradle.icm.ICMProjectPlugin.SETUP_DEVCARTRIDGES, "-s")
                .withGradleVersion(gradleVersion)
                .build()
    ...
    def 'check cartridge list with external cartridge'() {
        given:
        TestRepo repo = new TestRepo(new File(testProjectDir, "/repo"))
        String repoConf = repo.getRepoConfig()

        copyResources("cartridgelist", "build/tempfile")
        ...
        buildFile << """
            ...
            extendCartridgeList {
                templateFile.set(file("build/tempfile/cartridgelist.properties"))
            }
        ...
        writeJavaTestClass("com.intershop.testCartridge2", prj2dir)
        writeJavaTestClass("com.intershop.testCartridge3", prj3dir)
        writeJavaTestClass("com.intershop.testCartridge4", prj3dir)

        when:
        def result = getPreparedGradleRunner()
                .withArguments("extendCartridgeList", "-s")
                .withGradleVersion(gradleVersion)
                .build()
    ...
    def 'get lib filter file from configuration'() {
        given:
        TestRepo repo = new TestRepo(new File(testProjectDir, "/repo"))
        String repoConf = repo.getRepoConfig()
